...
<table className='container shadow'>
 ...
 history?.length>0?history.map((video,index)=>(
  <tr>
      <td className='text-danger fw-bolder'>{index+1}</td>
      <td className='fw-bolder'>{video.title}</td>
      <td><a href="{video.link}">{video.link}</a></td>
      <td>{video.timeStamp}</td>
      <td><button className='btn' onClick={e=>deleteHistory(video?.id)}><i class="fa-solid fa-trash text-danger"></i></button></td>
    </tr>
